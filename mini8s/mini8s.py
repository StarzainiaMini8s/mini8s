import pygame # Community edition
...
import zipfile
import shutil
GUST_PATTERN = re.compile(r'gust(?:ing|s)?\s+(?:to\s+)?(\d+)', re.IGNORECASE)
...
VERSION = "v0.3.99.2 [BETA]"
RELEASE_TAG = "mini8s-v0.3.99.2"
RELEASE_ARCH = "amd64" # SELF REMINDER TO CHANGE THIS FOR OTHER ARCHITECTURES
...
# Main/giant global configs blob.
previous_alerts = set()
...
LOGO_CONFIG = {"path": "textures/logos/mini8s_logo.png", "width": 175, "margin_right": 10, "margin_top": 10}
...
def get_coordinates_from_zip(zip_code):
    try:
        headers = {'User-Agent': 'mini8sv0.3.99family'}
        ...
        if timestamp_str:
            ...
            if gust_kmh is not None:
                ...
                if 'gusting' in wind_speed_str.lower() or 'gust' in wind_speed_str.lower():
                    ...
                    gust_match = GUST_PATTERN.search(wind_speed_str)
                    if gust_match:
...
def build_radar_url(lat, lon, zip_code=None):
    ...
    try:
        ...
        if ("TROPICAL" in alert_type) or ("HURRICANE" in alert_type) or ("SEVERE THUNDERSTORM WARNING" in alert_type) or ("SEVERE THUNDERSTORM WATCH" in alert_type):
            is_tropical = True
        elif alert_text_val and ("TROPICAL" in alert_text_val.upper() or "HURRICANE" in alert_text_val.upper() or "SEVERE THUNDERSTORM WARNING" in alert_text_val.upper() or "SEVERE THUNDERSTORM WATCH" in alert_text_val.upper()):
            is_tropical = True
    except Exception:
        pass
...
def get_weather_alerts(zip_code=None, state=None):
    try:
        ...
        for feature in features:
            ...
            # Check for tropical/hurricane/severe thunderstorm
            event_upper = event.upper() if event else ""
            if ("TROPICAL" in event_upper or "HURRICANE" in event_upper or "SEVERE THUNDERSTORM WARNING" in event_upper or "SEVERE THUNDERSTORM WATCH" in event_upper):
                is_tropical = True
...
def get_weather_icon_filename(condition, is_night=False, wind_speed=0, gust_speed=0):
    ...
    # Use the pre-loaded global config instead of loading from disk every time!
    config = WEATHER_ICON_CONFIG
    ...
    if not os.path.exists(icon):
        icon = "textures/icons/weather-none-available.png"
...
def create_forecast_panel_surface(forecast_periods, scaled_config, screen_width, panel_texture_cache, weather_icon_cache, font_cache):
    ...
    for i, period in enumerate(entries_to_draw):
        ...
        try:
            ...
            loaded_icon = pygame.image.load("textures/icons/weather-none-available.png").convert_alpha()
        if QUALITY_FACTOR < 1.0:
...
def create_all_pre_rendered_frames(radar_frames_list, conditions_panel_full, forecast_panel_full, title_text, mini8s_logo, logo_rect,
    ...
    for base_common_frame_with_radar in base_common_frames_global:
        if conditions_panel_full:
            ...
            pre_rendered_frames_conditions.append(frame_cond.convert_alpha())

        if forecast_panel_full:
            frame_fcst = base_common_frame_with_radar.copy()
            frame_fcst.blit(forecast_panel_full, panel_pos)
            pre_rendered_frames_forecast.append(frame_fcst.convert_alpha())
...
def draw_loading_screen(screen, message="Loading...", font_cache=None, motd_text=None, motd_y_position=None, scale_x=1.0, scale_y=1.0):
    ...
    try:
        version_logo = pygame.image.load("textures/logos/mini8s_logo_verstring.png").convert_alpha()
...
def is_running_from_source():
    """Check if Mini8s is running from source code or as a compiled binary"""
    # Check if we're frozen (running as PyInstaller binary)
    if getattr(sys, 'frozen', False):
        return False  # Running as binary
    else:
        return True  # Running from source

def get_current_version_clean():
    """Extract clean version from VERSION string (e.g., 'v0.3.99.1 [BETA]' -> 'v0.3.99.1')"""
    return VERSION.split()[0]

def get_current_release_tag():
    """Get the current release tag for comparison with GitHub releases"""
    return RELEASE_TAG

def check_for_updates():
    """
    Check GitHub API for the latest release.
    Returns dict with 'update_available', 'latest_version', 'download_url', 'error' on success/error.
    """
    current_release_tag = get_current_release_tag()
    current_version_display = get_current_version_clean()

    # Use /releases instead of /releases/latest to handle cases with no releases
    api_url = "https://api.github.com/repos/StarzainiaMini8s/mini8s/releases"
    ...
    try:
        ...
        # Get the latest release (first one, as GitHub returns them sorted by date)
        latest_release = releases[0]
        latest_tag = latest_release.get('tag_name', '')  # e.g., "mini8s-v0.3.99.1"
        ...
        # Strip "mini8s-" prefix from both for comparison
        latest_tag_normalized = latest_tag.replace('mini8s-', '', 1)
        current_tag_normalized = current_release_tag.replace('mini8s-', '', 1)

        # Simple string comparison (works for semver-like format: v0.3.99.1)
        if latest_tag_normalized > current_tag_normalized:
            download_url = get_platform_download_url(latest_release)
            if download_url:
                return {
                    'update_available': True,
                    'latest_version': latest_tag,  # Return full tag name
                ...
                return {
                    'update_available': False,
                    'error': 'no_binary',
                    'message': f'Update found ({latest_version}), but no compatible binary for this platform.'
                }
        else:
        ...
        if e.response.status_code == 404:
            return {
                'update_available': False,
                'error': '404',
                'message': 'Repository not found or has no releases.'
            }
        else:
            return {
                'update_available': False,
...
def get_platform_download_url(release_data):
    """
    Extract the appropriate download URL for the current platform and architecture.
    Uses RELEASE_ARCH to determine which binary to download.
    """
    ...
    # Determine architecture suffix based on RELEASE_ARCH
    if RELEASE_ARCH == 'amd64':
        arch_pattern = 'x64'
    else:  # arm64 or anything else
        arch_pattern = 'arm64'

    # Look for asset matching both OS and architecture
    # e.g., "mini8s-linux-x64.zip" or "mini8s-windows-x64.zip"
    for asset in assets:
        asset_name = asset.get('name', '').lower()
        if os_pattern in asset_name and arch_pattern in asset_name:
...
def get_update_filename():
    """Get the name for the downloaded update ZIP file"""
    return 'mini8s_update.zip'

def perform_update_swap():
    """
    Perform the actual update swap on startup.
    Extracts ZIP file, updates binary and all asset directories.
    Returns True if update was performed, False otherwise.
    """
    update_zip = get_update_filename()

    if not os.path.exists(update_zip):
        return False  # No pending update

    temp_extract_dir = 'mini8s_update_temp'

    try:
        # Extract ZIP to temporary directory
        print(f"Extracting update from {update_zip}...")
        with zipfile.ZipFile(update_zip, 'r') as zip_ref:
            zip_ref.extractall(temp_extract_dir)

        # The ZIP should contain a root folder (e.g., 'mini8s/')
        ...
        if len(extracted_contents) == 1 and os.path.isdir(os.path.join(temp_extract_dir, extracted_contents[0])):
            # ZIP has a root folder, use it
            source_dir = os.path.join(temp_extract_dir, extracted_contents[0])
            print(f"Using root folder: {source_dir}")
        else:
            # ZIP contents are directly in temp dir
            source_dir = temp_extract_dir
            print(f"Using temp dir directly: {source_dir}")

        # List what's in the source directory
        source_contents = os.listdir(source_dir)
        print(f"Source directory contents: {source_contents}")

        executable_name = get_executable_name()
        backup_name = f"{executable_name}_old"

        # Step 1: Backup current binary
        if os.path.exists(executable_name):
            print(f"Backing up current binary: {executable_name} -> {backup_name}")
            if os.path.exists(backup_name):
                os.remove(backup_name)
            os.rename(executable_name, backup_name)

        # Step 2: Copy new binary from extracted folder
        new_binary_path = os.path.join(source_dir, executable_name)
        print(f"Looking for new binary at: {new_binary_path}")

        if os.path.exists(new_binary_path):
            print(f"Found new binary, copying to: {executable_name}")
            # Get source binary size for verification
            source_size = os.path.getsize(new_binary_path)
            print(f"Source binary size: {source_size} bytes")

            shutil.copy2(new_binary_path, executable_name)

            # Verify copy succeeded
            if not os.path.exists(executable_name):
                raise Exception(f"Binary copy failed - {executable_name} does not exist after copy!")

            dest_size = os.path.getsize(executable_name)
            print(f"Destination binary size: {dest_size} bytes")

            if source_size != dest_size:
                raise Exception(f"Binary copy failed - size mismatch! Source: {source_size}, Dest: {dest_size}")

            # Make executable on Linux/macOS
            if platform.system() in ['Linux', 'Darwin']:
                import stat
                os.chmod(executable_name, os.stat(executable_name).st_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)
                # Verify it's executable
                if not os.access(executable_name, os.X_OK):
                    raise Exception(f"Failed to make {executable_name} executable!")
                print(f"Binary is executable: YES")

            print(f"Binary updated successfully!")
        else:
            # Binary not found - try to find it in subdirectories
            print(f"Binary not found at expected location, searching...")
            found = False
            for root, dirs, files in os.walk(source_dir):
                if executable_name in files:
                    ...
                    source_size = os.path.getsize(found_path)
                    print(f"Source binary size: {source_size} bytes")

                    shutil.copy2(found_path, executable_name)

                    dest_size = os.path.getsize(executable_name)
                    print(f"Destination binary size: {dest_size} bytes")

                    if source_size != dest_size:
                        raise Exception(f"Binary copy failed - size mismatch!")
                    ...
                    found = True
                    print(f"Binary updated successfully (via search)!")
                    break

            if not found:
                raise Exception(f"Binary '{executable_name}' not found in update ZIP at {source_dir}")

        # Step 3: Update all asset directories
        asset_dirs = ['textures', 'audio', 'fonts', 'var', 'tools']
        for asset_dir in asset_dirs:
            source_asset = os.path.join(source_dir, asset_dir)
            if os.path.exists(source_asset):
                print(f"Updating {asset_dir}/...")
                # Remove old directory if it exists
                if os.path.exists(asset_dir):
                    shutil.rmtree(asset_dir)
                # Copy new directory
                shutil.copytree(source_asset, asset_dir)
        ...
        # Remove old backup after successful update
        if os.path.exists(backup_name):
            try:
                print(f"Removing old backup: {backup_name}")
                os.remove(backup_name)
                print("Old backup removed successfully!")
            except Exception as remove_error:
                print(f"Warning: Could not remove old backup {backup_name}: {remove_error}")
                print("This is not critical - the update was successful.")

        # Verify the new binary exists
        if os.path.exists(executable_name):
            import stat
            file_stats = os.stat(executable_name)
            print(f"New binary verified at {executable_name} ({file_stats.st_size} bytes)")
        else:
            print(f"WARNING: Binary {executable_name} not found after update!")
        ...
        try:
            if os.path.exists(backup_name):
                if not os.path.exists(executable_name):
                    print(f"Restoring backup: {backup_name} -> {executable_name}")
                    os.rename(backup_name, executable_name)
                    print("Backup restored successfully!")
                else:
                    print(f"Binary exists, keeping backup at {backup_name}")
...
class HoverLabel(QLabel):
    def __init__(self, parent=None, hover_text="Advanced Settings"):
        super().__init__(parent)
        # Create the hover text label
        self.hover_text = QLabel(hover_text, parent)
...
class Mini8sWelcome(QMainWindow):
    def __init__(self):
        ...
        self.adv_settings_button = HoverLabel(self, "Advanced Settings")
        ...
        # Update icon
        self.update_button = HoverLabel(self, "Check for Updates")
        try:
            self.update_icon = QPixmap("textures/icons/update-mini8s.png")
            self.update_icon = self.update_icon.scaled(48, 48, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            self.update_button.setPixmap(self.update_icon)
        except Exception as e:
            print(f"Warning: Could not load update icon: {e}")
            # Fallback: create a simple colored square as placeholder
            fallback_pixmap = QPixmap(48, 48)
            fallback_pixmap.fill(Qt.blue)
            self.update_button.setPixmap(fallback_pixmap)
        self.update_button.setFixedSize(48, 48)
        ...
        self.update_button._mousePressEvent = self.check_and_download_update
        ...
        res_img = QPixmap("textures/setup/resolutions.png")
        ...
        zip_img = QPixmap("textures/setup/zipcode.png")
        ...
        start_button = ImageButton("textures/setup/start-button.png",
        ...
        try:
            logo_size = 225
            logo = QPixmap("textures/logos/mini8s_verstring-setup.png")
            ...
            # Position update button in top-left
            self.update_button.setParent(central_widget)
            self.update_button.move(5, 5)  # 5px from top and left
            ...
            build_date_label = QLabel("Build v0.3.99.2 (AMD64) compiled on 3/10/2026", central_widget)
    ...
    def check_and_download_update(self, event):
        """Handle the update button click - checks for updates and downloads if available"""
        # Step 1: Initial confirmation
        reply = QMessageBox.question(
            self,
            "Update Mini8s",
            "Are you sure you want to update Mini8s?",
        ...
        # Step 2: Check if running from source
        if is_running_from_source():
            QMessageBox.critical(
                self,
                "What did you think that would do!?",
                "You can't do updates if you're running off the source code, silly!"
            )
            return

        # Step 3: Check for updates in background thread
    ...
    def _start_update_check(self):
        ...
        self.progress_msg.setWindowTitle("Checking for Updates")
        self.progress_msg.setText(f"Searching for updates against the main repository...\n\nCurrent build: {get_current_version_clean()}")
        self.progress_msg.setStandardButtons(QMessageBox.NoButton)
        self.progress_msg.setModal(False)  # Non-modal to keep UI responsive
        ...
        def check_thread():
            self.update_check_result = check_for_updates()
        ...
        self.update_check_timer.timeout.connect(self._check_update_result)
        self.update_check_timer.start(100)  # Check every 100ms

    def _check_update_result(self):
        """Check if update check is complete and handle result"""
        if not self.update_check_done:
            return  # Still checking

        # Stop timer and close progress dialog
        self.update_check_timer.stop()
        self.progress_msg.close()

        update_info = self.update_check_result
        ...
        if update_info is None:
            QMessageBox.critical(
                self,
                "Update Check Failed",
                "Failed to check for updates. An unexpected error occurred."
            )
            return

        # Handle specific errors
        if 'error' in update_info:
            error_type = update_info['error']
            error_message = update_info.get('message', 'Unknown error occurred.')

            if error_type == 'no_releases':
                QMessageBox.information(
                    self,
                    "No Releases Available",
                    "No releases have been published for Mini8s yet.\n\nThe developer needs to create a release on GitHub first."
                )
            elif error_type == '404':
                QMessageBox.warning(
                    self,
        ...
        if not update_info.get('update_available', False):
            QMessageBox.information(
                ...
                f"You're already running the latest version of Mini8s!\n\nCurrent version: {get_current_version_clean()}"
            )
            return

        # Step 4: Update available - ask user if they want to download
        latest_version = update_info['latest_version']
        reply = QMessageBox.question(
            self,
            "Update Available",
            f"Mini8s has detected an update!\n\n{latest_version}\n\nWould you like to update?",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.Yes
        )
        ...
        # Step 5: Download the update in background thread
        self._start_update_download(update_info)

    def _start_update_download(self, update_info):
        """Start the update download in a background thread"""
        latest_version = update_info['latest_version']

        # Show download dialog
        self.download_msg = QMessageBox(self)
        self.download_msg.setWindowTitle("Downloading Update")
        self.download_msg.setText(f"Downloading Mini8s {latest_version}...\n\nPlease wait...")
        self.download_msg.setStandardButtons(QMessageBox.NoButton)
        self.download_msg.setModal(False)  # Non-modal to keep UI responsive
        ...
        def download_thread():
            ...
            self.download_result = {
                'success': download_update(update_info['download_url'], update_filename),
        ...
        self.download_timer.timeout.connect(self._check_download_result)
        self.download_timer.start(100)  # Check every 100ms

    def _check_download_result(self):
        """Check if download is complete and handle result"""
        ...
        # Step 6: Download successful - inform user to restart
        QMessageBox.information(
            self,
            "Update Downloaded",
            f"Update downloaded successfully!\n\nMini8s will now close. Please relaunch Mini8s to complete the update to {result['version']}."
        )
...
def initialize_mini8s():
    ...
    try:
        program_icon = pygame.image.load('textures/logos/mini8s_taskbar.png')
    ...
    ALERT_BAR_TEXTURE_PATH = "textures/graphics/warning_LDL.png"
    LOGO_CONFIG = {"path": "textures/logos/mini8s_logo.png", "width": 200, "margin_right": 10, "margin_top": 10}
    ...
    is_redmode = False
    logo_path = "textures/logos/mini8s_logo.png"
    try:
    ...
    try:
        title_4hr_normal_orig = pygame.image.load("textures/4hrtitles/4hrradar.png").convert_alpha()
        title_4hr_tropical_orig = pygame.image.load("textures/4hrtitles/4hrradarsatellite.png").convert_alpha()
        title_4hr_redmode_orig = pygame.image.load("textures/4hrtitles/4hrradar-redmode.png").convert_alpha()
        title_4hr_tropical_redmode_orig = pygame.image.load("textures/4hrtitles/4hrradarsatellite-redmode.png").convert_alpha()
        ...
        tropical_redmode_aspect = title_4hr_tropical_redmode_orig.get_width() / title_4hr_tropical_redmode_orig.get_height()
        ...
        tropical_redmode_width = int(target_height * tropical_redmode_aspect)
        ...
        if QUALITY_FACTOR < 1.0:
            ...
            intermediate_tropical_redmode_width = int(tropical_redmode_width * QUALITY_FACTOR)
            intermediate_tropical_redmode_height = int(target_height * QUALITY_FACTOR)
            ...
            title_4hr_tropical_redmode_temp = pygame.transform.smoothscale(title_4hr_tropical_redmode_orig, (intermediate_tropical_redmode_width, intermediate_tropical_redmode_height))
            title_4hr_tropical_redmode = pygame.transform.smoothscale(title_4hr_tropical_redmode_temp, (tropical_redmode_width, target_height))
        else:
            # Full quality
            ...
            title_4hr_tropical_redmode = pygame.transform.smoothscale(title_4hr_tropical_redmode_orig, (tropical_redmode_width, target_height))
    except Exception as e:
        ...
        title_4hr_tropical_redmode = None
        pygame.quit()
        sys.exit()
...
# Anti-freezing worker thread.. This is part of the logic that stops the loading screen from looking frozen
    ...
    while not init_data_received:
        ...
        try:
            ...
            if is_redmode:
                try:
                    redmode_logo_path = "textures/logos/mini8s-redmode_logo.png"
    ...
    # Initialize ticker variables — don't overwrite if already set during initialization
    if 'ticker_surface' not in locals():
        ticker_surface = None
        should_scroll = False
    if 'prev_ticker_text' not in locals():
        prev_ticker_text = None
        prev_ticker_font_key = None
        prev_ticker_width = 0
        prev_text_actual_width = 0
    running = True
    show_fps = SHOW_FPS
    is_transitioning = False
    panel_to_blit_during_flip = None
    ...
    while running:
        ...
        if is_transitioning:
            if transition_sub_step_idx < PANEL_FLIP_ANIMATION_STEPS:
                if current_ticks_ms - last_flip_sub_step_time > FLIP_SUB_STEP_DURATION_MS:
                    ...
                    if flip_anim_shrink and transition_sub_step_idx < len(flip_anim_shrink):
                        ...
                        panel_to_blit_during_flip = None
                        last_panel_switch_time = current_ticks_ms
                        continue
        ...
        try:
            ...
            if weather_data.get('lat') and weather_data.get('lon'):
                ...
                # Only clear title cache if we're implementing the tropical or redmode change
                if is_tropical != weather_data.get('is_tropical', False) or is_redmode != weather_data.get('is_redmode', False):
                    gradient_title_cache.clear()

                # Update is_tropical and is_redmode with the new values
                is_tropical = weather_data.get('is_tropical', False)
                is_redmode = weather_data.get('is_redmode', False)
                ...
                if alert_list:
                    ...
                    # Only recompute ticker surfaces/metrics when text or font key changes
                    if ticker_text_content != prev_ticker_text or ft_font_key != prev_ticker_font_key:
                        text_rect_ft = ft_font.get_rect(ticker_text_content)
                        text_actual_width = text_rect_ft.width
                        prev_ticker_text = ticker_text_content
                        prev_ticker_font_key = ft_font_key
                        ...
                        ticker_width = ticker_surface.get_width()
                        prev_ticker_width = ticker_width
                        prev_text_actual_width = text_actual_width
                    else:
                        # reuse previous values
                        ticker_width = prev_ticker_width
                        text_actual_width = prev_text_actual_width
        ...
        if active_radar_frame:
            ...
            try:
                ...
                if saved_config.get("quality") == "verylow":
                    ...
                    alpha = min_alpha + (max_alpha - min_alpha) * (math.sin(current_ticks_ms * pulse_speed) + 1) / 2

                # Set alpha directly on the surface (avoids expensive copy each frame)
                location_dot_original.set_alpha(int(alpha))
                dot_x = SCREEN_WIDTH // 2 - location_dot_original.get_width() // 2

                # This fucking dot.
                if SCREEN_WIDTH == 896 and SCREEN_HEIGHT == 504:
                    dot_y = (SCREEN_HEIGHT // 2) + (radar_offset_y // 3) - location_dot_original.get_height() // 2
                elif SCREEN_WIDTH == 1024 and SCREEN_HEIGHT == 600:
                    dot_y = (SCREEN_HEIGHT // 2) + (radar_offset_y // 2.5) - location_dot_original.get_height() // 2
                elif SCREEN_WIDTH == 960 and SCREEN_HEIGHT == 540:
                    dot_y = (SCREEN_HEIGHT // 2) + (radar_offset_y // 4.75) - location_dot_original.get_height() // 2
                elif SCREEN_WIDTH == 1024 and SCREEN_HEIGHT == 576:
                    dot_y = (SCREEN_HEIGHT // 2) + (radar_offset_y // 5.45) - location_dot_original.get_height() // 2
                else:
                    # For all other resolutions, use standard centering
                    dot_y = SCREEN_HEIGHT // 2 - location_dot_original.get_height() // 2

                screen.blit(location_dot_original, (dot_x, dot_y))
            except Exception as e:
                print(f"Error loading location dot: {e}")
        else:
                pass
        # Since images likely take less resources than gradient'd text.'
        if title_4hr_normal and title_4hr_tropical and title_4hr_redmode and title_4hr_tropical_redmode:
            if is_redmode and is_tropical and num_gifs > 1:
                title_image = title_4hr_tropical_redmode if current_gif_idx == 1 else title_4hr_redmode
            elif is_redmode and is_tropical:
                title_image = title_4hr_tropical_redmode
            elif is_redmode:
            ...
            if is_tropical and num_gifs > 1 and current_fade_alpha < 255:
                faded_title = title_image.copy()
                faded_title.set_alpha(current_fade_alpha)
...
if __name__ == "__main__":
    ...
    # Use Oxygen style for Frutiger Aero look
    app.setStyle("Oxygen")

    # Check for pending update on startup (only if running as binary)
    if not is_running_from_source():
        update_performed = perform_update_swap()
        if update_performed:
            # Show success message
            success_msg = QMessageBox()
            success_msg.setWindowTitle("Update Complete")
            success_msg.setText(
                f"Mini8s has successfully updated to {get_current_version_clean()}!\n\n"
                "Clicking \"OK\" will close Mini8s. Run Mini8s again and you will be on the newest version!"
            )
            success_msg.setIcon(QMessageBox.Information)
            success_msg.exec_()
            # Exit after showing message so user manually restarts
            sys.exit(0)
