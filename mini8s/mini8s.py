...
VERSION = "v0.3.98.1 [BETA]"
# Added an "Message of the Day". (with tropical and redmode varients too!)
# Completely removed 4:3 support. (very broken!) Only 16:9 resolutions will work (see the resolution169.json file for more details)
# Added a location dot.
# MOAR BUG FIXES!!!
...
def create_forecast_panel_surface(forecast_periods, scaled_config, screen_width, panel_texture_cache, weather_icon_cache, font_cache):
    ...
    ptext.draw("72 Hour Forecast", (padding, y_pos_draw), fontname=config["font_path"],
               ...
               ocolor=(0, 0, 0), owidth=2, surf=panel_surface)
...
def draw_loading_screen(screen, message="Loading...", font_cache=None, motd_text=None, motd_y_position=None, scale_x=1.0, scale_y=1.0):
    ...
    main_font_size = scale_font_size(80, scale_y)  # Increased from 48
    motd_font_size = scale_font_size(40, scale_y)  # Increased from 24
    version_font_size = scale_font_size(28, scale_y)  # Increased from 24

    # Draw main loading message with ptext (white text with black outline)
    ptext.draw(message,
               center=(display_width // 2, display_height // 2 - scale_value(80, scale_y)),  # Adjusted spacing
    ...
    if motd_text:
        ...
        is_urgent = any(word in motd_text.upper() for word in ["HURRICANE", "TROPICAL", "REDMODE", "DANGEROUS", "SHIT"])
        motd_color = (255, 50, 50) if is_urgent else (200, 200, 200)

        ptext.draw(motd_text,
    ...
    try:
        ...
        # Scale the logo based on screen resolution - make it larger
        logo_scale = min(scale_x, scale_y) * 0.7  # Increased from 0.5
...
def main():
    ...
    # Get ZIP code once (won't repeat if resolution is wrong)
    while True:
    ...
    while True:
        ...
        if allowed_resolutions and (SCREEN_WIDTH, SCREEN_HEIGHT) not in allowed_resolutions:
            print(f"\n⚠️ {SCREEN_WIDTH}x{SCREEN_HEIGHT} is not a 16:9 resolution!")
            continue  # Go back to width/height input
        break
    ...
    "WARNING_CONFIG": {
        ...
        "position": (scale_value(WARNING_CONFIG["position"][0], scale_x), SCREEN_HEIGHT - scaled_bottom_bar_height + scale_value(1, scale_y))
    },
    "TICKER_CONFIG": {
    ...
    if SCREEN_WIDTH > 1920:
        ...
        config_to_edit["title_font_size"] = int(config_to_edit["title_font_size"] * 0.9)
        config_to_edit["data_font_size"] = int(config_to_edit["data_font_size"] * 0.88)
        config_to_edit["list_font_size"] = int(config_to_edit["list_font_size"] * 0.9)
        config_to_edit["line_height"] = int(config_to_edit["line_height"] * 0.85)
    ...
    if alert_type_val:
        ...
        if "HURRICANE WATCH" in alert_upper or "HURRICANE WARNING" in alert_upper:
            ...
            # Update MOTD for tropical conditions
    ...
    while running:
        ...
        if active_radar_frame:
            ...
            radar_offset_y = int(base_offset * (SCREEN_HEIGHT / base_height) * multiplier)

            screen.blit(active_radar_frame, (0, radar_offset_y))

            # Add location dot in the middle of the visible radar area with pulsing effect
            try:
        ...
        if current_bar_texture:
            ...
            if warning_text and warning_text.strip():
                        # Draw warning text with ptext for outline effect
                        ptext.draw(warning_text,
